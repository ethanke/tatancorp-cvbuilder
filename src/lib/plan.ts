...
export interface AiCredits { remaining: number; total: number; used: number; }
export async function getUserPlan(cookie: string): Promise<PlanType> {
  try {
    const res = await fetch(`${BACKEND}/payments/status`, { headers: { cookie }, cache: "no-store" });
...
export async function getUserAiCredits(cookie: string): Promise<AiCredits> {
  try {
    const res = await fetch(`${BACKEND}/payments/ai/credits`, { headers: { cookie }, cache: "no-store" });
    if (!res.ok) return { remaining: 0, total: 3, used: 3 };
    return await res.json();
  } catch { return { remaining: 0, total: 3, used: 3 }; }
}
