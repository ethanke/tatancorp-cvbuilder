...
export default function CvPreview({ cv }: { cv: CVContent }) {
    return (
        <div id="cv-print-area" className="bg-white text-zinc-900 rounded-xl shadow-2xl p-10 text-[13px] leading-relaxed font-[system-ui,sans-serif] min-h-[29.7cm]" style={{ fontFamily: "Georgia, 'Times New Roman', serif" }}>
            <div className="border-b-2 border-zinc-800 pb-5 mb-6">
                <h1 className="text-3xl font-bold text-zinc-900">{cv.name || "Your Name"}</h1>
                {cv.tagline && <p className="text-base text-emerald-700 mt-0.5 font-medium">{cv.tagline}</p>}
                {cv.contact && (
            ...
            {cv.summary && <Section title="Summary"><p className="text-zinc-700">{cv.summary}</p></Section>}
                    ...
                    {cv.experience.map((exp, i) => (
                        <div key={i} className={i > 0 ? "mt-4" : ""}>
                            <div className="flex items-baseline justify-between">
                                ...
                                <p className="text-xs text-zinc-400 shrink-0 ml-4">{exp.start} – {exp.end}</p>
                            </div>
            ...
            {cv.skills?.length > 0 && (
                <Section title="Skills">
                    <div className="flex flex-wrap gap-2">
                        {cv.skills.map((s, i) => (
                            <span key={i} className="rounded-md border border-zinc-200 bg-zinc-50 px-2.5 py-0.5 text-xs text-zinc-700">{s}</span>
                        ))}
                    </div>
                </Section>
            )}
                    ...
                    {cv.projects.map((p, i) => (
                        <div key={i} className={i > 0 ? "mt-3" : ""}>
                            <div className="flex items-baseline gap-2">
                                ...
                                {p.url && <a href={p.url} className="text-xs text-emerald-700 hover:underline" target="_blank" rel="noreferrer">{p.url}</a>}
                            </div>
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
    return (
        <div className="mb-6">
            <h2 className="text-xs font-bold uppercase tracking-widest text-zinc-500 mb-3 border-b border-zinc-200 pb-1">{title}</h2>
            {children}
        </div>
    );
}
