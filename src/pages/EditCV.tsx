...
export default function BuilderEditor() {
    ...
    const handleTailor = async () => {
        ...
        setTailoring(true);
        try {
            const res = await fetch(`${BACKEND}/cv/${id}/tailor`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                credentials: "include",
            ...
            if (!res.ok) {
                if (data.code === "ai_credits_exhausted") { setShowUpgradeModal(true); setTailoring(false); return; }
                if (data.code === "PLAN_REQUIRED") { setShowUpgradeModal(true); setTailoring(false); return; }
                throw new Error(data.error || "Tailor failed");
            }
            if (data.newId) { navigate(`/builder/${data.newId}`); }
        } catch (e: unknown) {
    ...
    const handleShare = async () => {
        const publicUrl = `${import.meta.env.VITE_APP_URL ?? "https://cvbuilder.tatancorp.xyz"}/cv/${id}`;
        try {
            if (!cv?.is_public) {
            ...
            <div className="print:hidden flex items-center gap-4 px-6 py-3 border-b border-zinc-800 bg-[#09090b]/80 backdrop-blur-md shrink-0">
                <div className="flex-1 min-w-0">
                    <input value={cv.title} onChange={(e) => setCv({ ...cv, title: e.target.value })}
                        onBlur={() => {
                            fetch(`${BACKEND}/cv/${id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, credentials: "include", body: JSON.stringify({ title: cv.title }) });
                        }}
                ...
                <span className={`text-xs shrink-0 ${saveStatus === "saved" ? "text-emerald-500" : saveStatus === "saving" ? "text-zinc-400" : "text-amber-400"}`}>
                    {saveStatus === "saved" ? "✓ Saved" : saveStatus === "saving" ? "Saving…" : "Unsaved"}
                </span>
                <button onClick={handleShare} className="shrink-0 rounded-lg border border-zinc-700 bg-zinc-900 px-3 py-1.5 text-xs font-medium hover:bg-zinc-800 transition">↗ Share</button>
                <button onClick={() => setTailorOpen(true)} className="shrink-0 rounded-lg border border-zinc-700 bg-zinc-900 px-3 py-1.5 text-xs font-medium hover:bg-zinc-800 transition">
                    ⌖ Tailor to job {creditsExhausted && <span className="text-zinc-500">(no credits)</span>}
                </button>
                <button onClick={() => navigate(`/builder/${id}/cover-letter`)} className="shrink-0 rounded-lg border border-zinc-700 bg-zinc-900 px-3 py-1.5 text-xs font-medium hover:bg-zinc-800 transition">
                    ✦ Cover Letter {plan === "free" && <span className="text-zinc-500">(Pro)</span>}
                </button>
                <button onClick={() => window.print()} className="shrink-0 rounded-lg bg-emerald-500 px-3 py-1.5 text-xs font-semibold text-black hover:bg-emerald-400 transition">⤓ Download PDF</button>
            </div>
                ...
                <div className="w-105 shrink-0 overflow-y-auto border-r border-zinc-800 p-6 flex flex-col gap-8">
                    ...
                    <section>
                        <div className="flex items-center justify-between mb-4">
                            ...
                            <button onClick={() => update({ experience: [...content.experience, { company: "", role: "", start: "", end: "present", bullets: [""] }] })} className="text-xs text-emerald-400 hover:text-emerald-300">+ Add</button>
                        </div>
                        {content.experience.map((exp, i) => (
                            <ExperienceCard key={i} exp={exp}
                                onChange={(e) => { const next = [...content.experience]; next[i] = e; update({ experience: next }); }}
                    ...
                    <section>
                        <div className="flex items-center justify-between mb-4">
                            ...
                            <button onClick={() => update({ education: [...content.education, { school: "", degree: "", field: "", year: "" }] })} className="text-xs text-emerald-400 hover:text-emerald-300">+ Add</button>
                        </div>
                        {content.education.map((edu, i) => (
                            <EducationCard key={i} edu={edu}
                                onChange={(e) => { const next = [...content.education]; next[i] = e; update({ education: next }); }}
                    ...
                    <section>
                        ...
                        <div className="flex flex-col gap-1">
                            ...
                            <input value={content.skills.join(", ")} onChange={(e) => update({ skills: e.target.value.split(",").map((s) => s.trim()).filter(Boolean) })}
                    ...
                    <section>
                        <div className="flex items-center justify-between mb-4">
                            ...
                            <button onClick={() => update({ projects: [...content.projects, { name: "", description: "", url: null }] })} className="text-xs text-emerald-400 hover:text-emerald-300">+ Add</button>
                        </div>
                        {content.projects.map((proj, i) => (
                            <ProjectCard key={i} proj={proj}
                                onChange={(p) => { const next = [...content.projects]; next[i] = p; update({ projects: next }); }}
            ...
            {tailorOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
                    <div className="w-full max-w-xl rounded-2xl border border-zinc-800 bg-zinc-900 p-6 flex flex-col gap-5">
                        ...
                        <p className="text-sm text-zinc-400">Paste the job description below. The AI will create a <strong className="text-white">new tailored copy</strong> of your CV — your original is untouched.</p>
                        <textarea value={jobDesc} onChange={(e) => setJobDesc(e.target.value)} maxLength={4000} rows={8}
                        ...
                        {tailorError && <p className="text-sm text-red-400 bg-red-500/10 border border-red-500/20 rounded-lg px-3 py-2">{tailorError}</p>}
                        <div className="flex gap-3 justify-end">
                            <button onClick={() => setTailorOpen(false)} className="px-4 py-2 text-sm text-zinc-400 hover:text-white transition">Cancel</button>
                            <button onClick={handleTailor} disabled={tailoring || !jobDesc.trim()} className="rounded-xl bg-emerald-500 px-5 py-2 text-sm font-semibold text-black hover:bg-emerald-400 disabled:opacity-60 transition">
