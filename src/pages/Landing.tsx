...
const features = [
    { icon: "✦", title: "Generate from scratch", desc: "Paste a short bio and target role. GPT-4o writes a complete, polished CV in under 30 seconds." },
    { icon: "↑", title: "Improve existing CV", desc: "Have a rough draft? Paste it in and the AI rewrites every section with stronger language and structure." },
    { icon: "⌖", title: "Tailor to any job", desc: "Paste a job description and get a targeted version of your CV that matches the required skills and keywords." },
    { icon: "⤓", title: "Export to PDF", desc: "One click. Clean, print-ready A4 PDF — no watermarks, no subscriptions." },
];
        ...
        <div className="mx-auto max-w-6xl px-6 py-20 flex flex-col gap-24">
            ...
            <section className="flex flex-col items-center text-center gap-6">
                ...
                <h1 className="text-5xl md:text-6xl font-bold tracking-tight text-white leading-tight max-w-3xl">
                    Your CV, written by <span className="text-emerald-400">AI</span> in seconds
                </h1>
                ...
                <div className="flex items-center gap-4 pt-2">
                    <Button asChild className="rounded-xl bg-emerald-500 px-7 py-3 text-base font-semibold text-black transition hover:bg-emerald-400 h-auto">
                        <a href={`${BACKEND}/auth/login?next=${encodeURIComponent("https://cvbuilder.tatancorp.xyz/dashboard")}`}>
                            Build my CV — free
                        </a>
                    </Button>
                    <Link to="#features" className="text-sm text-zinc-400 hover:text-white transition">
                        See how it works →
                    </Link>
                </div>
            </section>
            ...
            <section className="relative rounded-2xl border border-zinc-800 bg-zinc-900/50 overflow-hidden p-8">
                ...
                <div className="grid md:grid-cols-2 gap-8 relative">
                    ...
                    <div className="rounded-xl border border-zinc-700 bg-white p-5 text-zinc-900 text-xs leading-relaxed shadow-xl">
                        ...
                        <p className="mb-3">Results-driven engineer with 4 years of experience building scalable React frontends and Node.js APIs. Led a team of 3 engineers shipping a payments product to 50k+ users at a fintech startup.</p>
            ...
            <section id="features" className="flex flex-col gap-8">
                ...
                <div className="grid sm:grid-cols-2 gap-5">
                    {features.map((f, idx) =>
                        idx === 0 ? (
                            <Card key={f.title} className="rounded-2xl border border-zinc-800 bg-zinc-900/50 py-0">
            ...
            <section id="pricing" className="flex flex-col gap-8">
                ...
                <div className="flex items-center justify-center gap-3">
                    <span className={`text-sm font-medium transition ${billing === "monthly" ? "text-white" : "text-zinc-500"}`}>Monthly</span>
                    <button onClick={() => setBilling(billing === "monthly" ? "annual" : "monthly")} aria-label="Toggle billing period" className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none ${billing === "annual" ? "bg-emerald-500" : "bg-zinc-700"}`}>
                    ...
                    <span className={`text-sm font-medium transition ${billing === "annual" ? "text-white" : "text-zinc-500"}`}>Annual</span>
                    {billing === "annual" && (
                        <span className="rounded-full bg-emerald-500/15 border border-emerald-500/30 px-2.5 py-0.5 text-xs font-semibold text-emerald-400">Save ~18%</span>
                    )}
                </div>
                <div className="grid sm:grid-cols-2 gap-5 max-w-3xl mx-auto w-full">
                    <div className="rounded-2xl border border-zinc-800 bg-zinc-900/50 p-6 flex flex-col gap-4">
                        ...
                        <a href={`${BACKEND}/auth/login?next=${encodeURIComponent("https://cvbuilder.tatancorp.xyz/dashboard")}`} className="mt-auto rounded-xl border border-zinc-700 px-5 py-2.5 text-sm font-semibold text-white text-center transition hover:bg-zinc-800">
                            Get started
                        </a>
                    </div>
                    <div className="rounded-2xl border border-emerald-500/40 bg-emerald-500/5 p-6 flex flex-col gap-4 relative">
                        {billing === "annual" && (
                            <div className="absolute -top-3 right-5 rounded-full bg-emerald-500 px-3 py-0.5 text-xs font-semibold text-black">Best value</div>
                        )}
                        <p className="text-xs text-emerald-400 uppercase tracking-widest font-medium">{billing === "annual" ? "Pro — Annual" : "Pro — Monthly"}</p>
                        <div className="flex items-baseline gap-1">
                            {billing === "annual" ? (
                                <><span className="text-4xl font-bold text-white">$49</span><span className="text-sm text-zinc-500">/year</span></>
                            ) : (
                                <><span className="text-4xl font-bold text-white">$5</span><span className="text-sm text-zinc-500">/month</span></>
                            )}
                        </div>
                        <ul className="text-sm text-zinc-300 flex flex-col gap-2 mt-2">
                        ...
                        <a href={`${BACKEND}/auth/login?next=${encodeURIComponent(`https://cvbuilder.tatancorp.xyz/dashboard?upgrade=1&plan=${billing}`)}`} className="mt-auto rounded-xl bg-emerald-500 px-5 py-2.5 text-sm font-semibold text-black text-center transition hover:bg-emerald-400">
            ...
            <section className="flex flex-col items-center gap-5 text-center py-8">
                ...
                <a href={`${BACKEND}/auth/login?next=${encodeURIComponent("https://cvbuilder.tatancorp.xyz/dashboard")}`} className="rounded-xl bg-emerald-500 px-8 py-3 text-base font-semibold text-black transition hover:bg-emerald-400">
